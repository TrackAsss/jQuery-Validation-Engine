define('validation/js/jquery.validationEngine',['jquery'], function($){
	/*
	 ...
	 (function($) {
		 ...
		 var methods = {
			 ...
			 */
			 init: function(options) {
				 ...
				 if (!form.data('jqv') || form.data('jqv') == null ) {
					...
					var $this = $(form).validationEngine;
					var form = $(form);
					...
					$(this).find('[data-validation-engine*=rfuncCall]').each(function(index, el){
					 	...
					 	$.each(rules, function(index, rule){
					 		if( rule.substring(0,9) == 'rfuncCall' ){
					 			rCalls.push(rule);
					 		}
					 	});
					 	...
					 	$.each(rCalls,function(index,rCall){
					 		...
					 		var namespace = func.split('.')[0];
					 		if( !options.validators[namespace] ){
					 			options.validators[namespace] = true;
					 			var val = new $.Deferred();
						 		validators.push(val);
						 		var ns = namespace+'/validators';
						 		require([ns], function(validator){
						 			options.validators[namespace] = validator;
						 			val.resolve();
						 		});
					 		}
					 	});
			...
			*/
			validate: function() {
				var $sthis = $(this);
				if( !$sthis.is('form') ) $sthis = $sthis.closest('form');
				$sthis = $($sthis).validationEngine;
				var $this = this;
				var element = $(this);
				$sthis.valid = new $.Deferred();
				$sthis.validators_loaded.done(function(){
			...
			*/
			_rfuncCall: function(field, rules, i, options) {
				var functionName = rules[i + 1];
				var namespaces = functionName.split('.');
				return options.validators[namespaces[0]][namespaces[1]](field, rules, i, options);
			},
			/**
			* Field match
			*
